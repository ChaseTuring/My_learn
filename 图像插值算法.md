...
if __name__ == "__main__":
    img = cv2.imread("E:\\Downloads\\1.png")
    print(img.shape)
    s_percent = 0.3
    shape = img.shape
    width = shape[1]
    height = shape[0]
    width = int(width * s_percent)
    height = int(height * s_percent)
    dim = (width, height)
    resized1 = cv2.resize(img, dim, interpolation=cv2.INTER_NEAREST)
    resized2 = cv2.resize(img, dim, interpolation=cv2.INTER_LINEAR)
#     resized1 = cv2.resize(img, dsize=None, fx=s_percent, fy=s_percent, interpolation=cv2.INTER_NEAREST)
#     resized2 = cv2.resize(img, dsize=None, fx=s_percent, fy=s_percent, interpolation=cv2.INTER_LINEAR)
    l_percent = 1.5
    width = int(width * l_percent)
    height = int(height * l_percent)
    dim1 = (width, height)
#     resized3 = cv2.resize(img, dim1, cv2.INTER_NEAREST)
#     resized4 = cv2.resize(img, dim1, cv2.INTER_LINEAR)
    resized3 = cv2.resize(img, dsize=None, fx=l_percent, fy=l_percent, interpolation=cv2.INTER_NEAREST)
    resized4 = cv2.resize(img, dsize=None, fx=l_percent, fy=l_percent, interpolation=cv2.INTER_LINEAR)
